...
export const ShopOwnerColumns: ColumnDef<UsersTableDataType>[] = [
  ...
  },
  {
    ...
    cell: function Cell({ row }) {
      return row.original.owner?.shop?.name;
    },
  },
  {
    id: "actions",
    cell: function Cell({ row }) {
      ...
      const handleDeleteUser = async () => {
        ...
        try {
          ...
          if (result.success) {
            toast.success(result.message || "Pemilik kedai berhasil dihapus!");
          } else {
            toast.error(result.error?.message || "Gagal menghapus pemilik kedai.");
          }
        } catch (error) {
          console.error("Error deleting user:", error);
          toast.error("Terjadi kesalahan tak terduga saat menghapus pemilik kedai.");
        } finally {
          setIsDeleting(false);
          ...
          <AlertDialog open={isConfirmDialogOpen} onOpenChange={setIsConfirmDialogOpen}>
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  Apakah Anda yakin ingin menghapus pemilik kedai &quot;{user.name}&quot;? 
